...
Ref2 = pd.DataFrame({'SHA': sha, 'Message': message},
                    columns=['SHA', 'Message'])
...
def preprocessing(list):
    ...
    for message in list:
        ...
        message = "".join(
            [char for char in message if char not in ReturnToTheLine])
...
dataset_array_list = sentence_embeding(
    word_embeding(preprocessing(dataset['Message'].tolist())))
Ref2_array_list = sentence_embeding(
    word_embeding(preprocessing(Ref2['Message'].tolist())))
dataset_dist_list = []
...
input = pd.concat([dataset, pd.DataFrame({'Distance': dataset_dist_list, 'Distance1': dataset_meandist_list}, columns=[
                  'Distance', 'Distance1'])], axis=1, join="inner")
...
feature_cols = ['Distance', 'Additions', 'Deletions', 'Total']
